...
% Load audio signals
[primarySignal, fs1] = audioread(sprintf('Bandpass_output_mic%d.wav', inputIndex));
[referenceNoise, fs2] = audioread(sprintf('Bandpass_output_mic6.wav'));
...
% Save the filtered signal as a new audio file (use audiowrite)
save_file_name = sprintf('Anc_output_mic%d.wav', inputIndex);
audiowrite(save_file_name, outputSignal,fs1);  % This writes the signal as .wav
